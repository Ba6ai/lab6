...
#### В классе Point создаются поля типа double _x, _y; конструктор с параметрами (передаваемые из Main и присвоение к своим полям класса), конструктор копирования(передаётся объект класса и его поля присваиваются в текущие), свойства X, Y происходит чтение(отдаёт числа наружу) и запись нового значения(сохраняет новое число в поле), перегрузка метода ToString(), поиск расстояния - GetDistance(метод вызывается у объекта первой точки, использует свои this._x и this._y и координаты переданного объекта. Создаются переменные типа double x1, y1, x2, y2; пользователь вводит значения в эти переменные(проверка ввода осуществляется через TryParse), создаются 2 объекта класса Point с присваиванием значений из переменных. Вывод расстояния между двумя точками через формулу.



#### В классе Point создаются поля типа double _x, _y; конструктор с параметрами (передаваемые из Main и присвоение к своим полям класса), конструктор копирования(передаётся объект класса и его поля присваиваются в текущие), свойства X, Y происходит чтение(отдаёт числа наружу) и запись нового значения(сохраняет новое число в поле), перегрузка метода ToString(), вызов поиска расстояния GetDistance с полями первой точки, а в качестве параметра - поля второй точки.
