...
\#### В классе MAX происходит создание коструктора по умолчанию "public MAX(int x,int y,int z)" и передаёт ему цифры из Main присваивая их к своим переменным "this.x = x" итд. Далее описывается конструктор копирования "public MAX(MAX i)" для нового объекта(у меня в коде это count2 в Main). Далее свойство используещее копирование для замены "x" не изменяя значчения count1 "public getX". После описывается получение максимума используя математическую функция "Math". В самом конце происходит перегруз метода ToString()
